        ...
        {savedList.map((item, index) => {
          ...
          return (
            <motion.div
              ...
              className="group relative flex flex-col sm:flex-row sm:items-center justify-between gap-2 rounded-lg border border-hairline bg-canvas p-4.5 shadow-[0_1px_2px_rgba(0,0,0,0.01)] hover:border-neutral-400 hover:shadow-[0_4px_12px_rgba(0,0,0,0.02)] transition-all duration-200 cursor-pointer"
            >
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
                  <span className="inline-flex items-center gap-1.5 font-sans font-semibold text-ink text-sm group-hover:text-link transition-colors">
                    {item.analysis ? (
                      ...
                      <FileText size={14} className="text-mute group-hover:text-link transition-colors" />
                    )}
                    {item.fullName}
                  </span>
                  <div className="flex flex-wrap gap-1">
                    {badgeItems.map((badge, index) => (
                      <span key={`${item.id}-${badge}-${index}`} className="rounded px-1.5 py-0.5 text-[10px] font-medium text-body border border-hairline bg-canvas-soft-2/80 shadow-[inset_0_1px_1.5px_rgba(0,0,0,0.03)]">
                        {badge}
                      </span>
                    ))}
                  </div>
              ...
              <div className="flex items-center justify-between sm:justify-end gap-4 border-t border-hairline/60 sm:border-0 pt-2 sm:pt-0 shrink-0">
